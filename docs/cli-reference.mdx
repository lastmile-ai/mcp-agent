...
- `mcp-agent deploy <app>` → `mcp-agent cloud deploy <app>`
...
| Command | Description | Example |
| :------ | :---------- | :------ |
| `login` | Authenticate | `mcp-agent cloud auth login` |
| `whoami` | Show current user | `mcp-agent cloud auth whoami` |
| `logout` | Sign out | `mcp-agent cloud auth logout` |
...
```bash
mcp-agent cloud deploy <APP_NAME> [OPTIONS]
```
...
| `--api-url` | API base URL | From env or config |
| `--api-key` | API key | From env or config |

**Examples:**
```bash
# Simple deployment
mcp-agent cloud deploy my-agent
...
# Dry run
mcp-agent cloud deploy my-agent --dry-run
```
...
| :------- | :---------- |
| `MCP_API_BASE_URL` | Cloud API endpoint |
...
# Deploy to cloud
mcp-agent cloud deploy production-agent
...
# Verbose deployment
mcp-agent cloud deploy my-agent --verbose
