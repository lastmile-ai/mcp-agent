...
- If `.mcpacignore` exists in the working (or `--config-dir`) directory, it is applied automatically.
- `--ignore-file <path>` overrides the default detection when you need a different ignore file.
