...
| `--ignore` | Use `.mcpacignore` in current directory to ignore files | `false` |
| `--ignore-file <path>` | Specify a custom ignore file (gitignore syntax) | None |
...
# Use default ignore file in CWD
...
- `--ignore` looks for `.mcpacignore` in the current working directory.
- `--ignore-file <path>` specifies a custom ignore file and does not require `--ignore`.
- If both are provided, `--ignore-file` takes precedence.
