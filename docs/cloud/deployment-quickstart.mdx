...
To inspect or rotate deployment-time environment secrets later, use `uvx mcp-agent cloud secrets list|add|remove|pull`. These commands operate on the same handles created during deploy and are safe to run from CI.
