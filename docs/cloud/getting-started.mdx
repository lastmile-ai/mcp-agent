...
<Info>
  MCP Agent Cloud is currently in open beta. Follow the instructions below to get started, or reach out to team@lastmileai.dev for feedback.
</Info>
...
<Steps>
  ...
  <Step title="Deploy">
    ...
    2. Process your secrets file (transforming secret tags to handles)
    3. Bundle and upload your code
    4. Deploy to MCP Agent Cloud
    
    You'll see output like:
    ```
    App ID: app_abc123xyz
    App URL: https://api.mcp-agent.cloud/servers/my-first-agent
    App Status: ONLINE
    ```
  </Step>
...
<Tabs>
  <Tab title="Claude Desktop">
    ...
    ```bash
    mcp-agent configure --id https://deployments.mcp-agent.com/servers/web-summarizer
    ```
...
```bash
# Set API base URL
export MCP_API_BASE_URL="https://api.mcp-agent.cloud"
...
<Note>
The web console is coming soon. For now, use the CLI commands to manage your deployments.
</Note>
...
mcp-agent configure --id https://api.mcp-agent.cloud/servers/my-agent

# Show required parameters for configuration
mcp-agent configure --id https://api.mcp-agent.cloud/servers/my-agent --params
```
