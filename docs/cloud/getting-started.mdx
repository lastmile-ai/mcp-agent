...
<Steps>
  ...
  <Step title="Deploy">
    ...
    2. Process your secrets file (transforming secrets to secure handles)
...
MCP Agent Cloud securely handles secrets through the `mcp_agent.secrets.yaml` file. During deployment, you will be prompted to specify 
which secrets should securely uploaded to MCP Agent Cloud's vault to be used for the deployed app and which secrets will be excluded 
(and require a subsequent 'configure' command to configure with user secrets).
