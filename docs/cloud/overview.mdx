...
<Note>You can optionally exclude files from the bundle using an ignore file (gitignore syntax). Use `--ignore` to look for `.mcpacignore` in your project directory, or `--ignore-file <path>` to specify a different file. `--ignore-file` can be used on its own and takes precedence if both are provided.</Note>
