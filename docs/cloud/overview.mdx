...
<Note>You can optionally exclude files from the bundle using an ignore file (gitignore syntax). If `.mcpacignore` exists in your project directory it is applied automatically, or pass `--ignore-file <path>` to point to a different file.</Note>
