...
<Tip>
Use `uv run mcp-agent config builder` for an interactive wizard that generates both config and secrets files. Run `uv run mcp-agent config show --secrets` at any time to see what the CLI discovers.
</Tip>
