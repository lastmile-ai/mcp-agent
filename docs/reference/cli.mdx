...
The `mcp-agent` CLI is the primary interface for project scaffolding, deployment, and day‑to‑day management of MCP Agent Cloud. Install it globally with `uv tool install mcp-agent`, or run commands in-place with `uv run mcp-agent …` so they execute inside your project environment.
...
| :------ | :------ | :------ |
| `uv run mcp-agent login` | Authenticate with MCP Agent Cloud. | `uv run mcp-agent login --no-open` |
| `uv run mcp-agent init` | Scaffold a project from a template. | `uv run mcp-agent init --template basic --dir apps/agent` |
| `uv run mcp-agent init --quickstart` | Copy a curated quickstart example. | `uv run mcp-agent init --quickstart workflow --dir ./workflow-demo` |
| `uv run mcp-agent deploy <APP_NAME>` | Package the current directory and deploy it. | `uv run mcp-agent deploy research-agent --non-interactive` |
| `uv run mcp-agent cloud servers list` | List deployed or configured servers. | `uv run mcp-agent cloud servers list --filter prod --format json` |
| `uv run mcp-agent install --client claude_code <URL>` | Register a deployed server with a local MCP client. | `uv run mcp-agent install https://srv_abc.deployments... --client claude_code` |
...
After authenticating, `uv run mcp-agent cloud auth whoami` confirms the active account. Use `uv run mcp-agent cloud auth logout` to clear stored credentials.
...
| `mcp-basic-agent` | Finder agent from the README, ready to run via `uv run main.py`. |
...
```bash
uv run mcp-agent init --template basic --dir apps/research-agent
uv run mcp-agent init --quickstart workflow --dir ./workflow-demo --force
uv run mcp-agent init --list
```

### Deploy an application (`mcp-agent deploy`)

```bash
uv run mcp-agent deploy research-app \
  --config-dir ./deploy \
...
```bash
uv run mcp-agent cloud servers list --filter prod --sort-by -created --format json
uv run mcp-agent cloud servers describe srv_123
```
...
```bash
uv run mcp-agent install https://srv_abc.deployments.mcp-agent.com \
...
uv run mcp-agent cloud servers list

# Tail logs for a specific deployment
uv run mcp-agent cloud logger tail srv_abc123 --follow

# Resume a paused workflow run
uv run mcp-agent cloud workflows resume srv_abc123 run_xyz789

# Review required secrets before configuring an app
uv run mcp-agent cloud configure --id https://srv_abc123.deployments.mcp-agent.com/mcp --params
```
