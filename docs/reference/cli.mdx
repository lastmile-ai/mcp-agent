...
- **Secrets (`mcp-agent cloud secrets …`)** – manage environment secrets captured during deploy.  
  ...
  - `secrets add <KEY> <VALUE> [APP]` creates or updates the secret value (`apps/<app_id>/env/<KEY>`).  
  - `secrets remove <KEY> [APP]` deletes the stored handle.  
  - `secrets pull [APP] --output mcp_agent.cloud.secrets.yaml` downloads resolved values without touching `mcp_agent.secrets.yaml`.
