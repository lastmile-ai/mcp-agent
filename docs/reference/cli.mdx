...
| `cloud secrets list` | Inspect deployment environment secrets. | `uvx mcp-agent cloud secrets list my-app` |
...
- **Secrets (`mcp-agent cloud secrets …`)** – manage environment secrets captured during deploy.  
  ...
  - `secrets add <KEY> <VALUE> [APP]` (or `--from-env-file .env.local`) creates or updates secret values (`apps/<app_id>/env/<KEY>`).  
  - `secrets remove <KEY> [APP]` deletes the stored handle.  
  - `secrets pull [APP] --format env|yaml --output <FILE>` downloads resolved values as either a dotenv file (default `.env.mcp-cloud`) or a YAML file. `.env` is loaded first (developer overrides), followed by `.env.mcp-cloud`, before config-driven fallbacks are applied.
