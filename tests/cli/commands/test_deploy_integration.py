...
def test_deploy_e2e_with_env_vars(real_world_example_dir, mock_secrets_client):
    ...
    # Set up paths
    config_path = Path(real_world_example_dir) / "mcp_agent.config.yaml"
    secrets_path = Path(real_world_example_dir) / "mcp_agent.secrets.yaml"
    output_path = (
        Path(real_world_example_dir) / "mcp_agent.deployed.secrets.yaml"
    )
...
def test_deploy_e2e_with_no_secrets(real_world_example_dir):
    ...
    # Verify the result is mock app ID
...
def test_deploy_e2e_with_dry_run(real_world_example_dir):
    ...
    with patch.dict(os.environ, {"BEDROCK_KEY": "dev-bedrock-key-from-env"}):
    ...
    # Verify the result is the mock app ID
    assert result == MOCK_APP_ID
