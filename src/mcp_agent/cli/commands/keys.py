...
PROVIDERS = {
    ...
    "bedrock": {
        ...
        "models": ["anthropic.claude-3-sonnet", "anthropic.claude-3-haiku", "amazon.titan"],
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            if provider == "google":
                ...
                response = await client.get(config["test_endpoint"], headers=headers, timeout=5)
...
def set_key(
    ...
    env_only: bool = typer.Option(False, "--env-only", help="Set in environment only, not secrets file"),
) -> None:
...
def unset(
    ...
    if sec_path and sec_path.exists():
        try:
            ...
            console.print(f"[yellow]Warning: Could not update secrets file: {e}[/yellow]")
