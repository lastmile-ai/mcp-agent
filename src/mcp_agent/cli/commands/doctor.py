...
def _generate_suggestions(
    ...
    if not commands["npx"] and any(
        s.command == "npx" for s in (servers.values() if isinstance(servers, dict) else servers)
    ):
        suggestions.append(
    ...
    if settings.otel and settings.otel.enabled:
        try:
            for e in settings.otel.exporters or []:
                if getattr(e, "type", None) == "otlp" and not getattr(e, "endpoint", None):
                    suggestions.append(
...
def doctor() -> None:
    ...
    files_table.add_row(
        "Secrets",
        secrets_status_display,
        str(secrets_path) if secrets_path else "-"
    )
    ...
    # OTEL Configuration
    if settings.otel and settings.otel.enabled:
        otel_table = Table(title="OpenTelemetry Configuration", show_header=False, box=None)
