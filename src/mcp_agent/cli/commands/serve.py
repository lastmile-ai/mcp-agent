...
def serve(
    ctx: typer.Context,
    script: Optional[str] = typer.Option(None, "--script", "-s", help="Python script with MCPApp"),
    transport: str = typer.Option("stdio", "--transport", "-t", help="Transport: stdio|http|sse"),
    port: Optional[int] = typer.Option(None, "--port", "-p", help="Port for HTTP/SSE server"),
    host: str = typer.Option("0.0.0.0", "--host", "-h", help="Host for HTTP/SSE server"),
    reload: bool = typer.Option(False, "--reload", "-r", help="Auto-reload on code changes"),
    debug: bool = typer.Option(False, "--debug", "-d", help="Enable debug mode"),
    workers: int = typer.Option(1, "--workers", "-w", help="Number of worker processes (HTTP only)"),
    env: Optional[List[str]] = typer.Option(None, "--env", "-e", help="Environment variables (KEY=value)"),
    config: Optional[Path] = typer.Option(None, "--config", "-c", help="Config file path"),
    show_tools: bool = typer.Option(False, "--show-tools", help="Display available tools on startup"),
    monitor: bool = typer.Option(False, "--monitor", "-m", help="Enable monitoring dashboard"),
) -> None:
    """
    ...
    if config:
        os.environ["MCP_AGENT_CONFIG"] = str(config)
        ...
        if transport == "stdio":
            ...
            try:
                ...
                if monitor and server_monitor:
                    server_monitor.start_time = asyncio.get_event_loop().time()
...
def generate(
    ...
    output: Path = typer.Option(Path("server.py"), "--output", "-o", help="Output file"),
    ...
    try:
        with resources.files("mcp_agent.data.templates").joinpath(template_file).open() as f:
            content = f.read()
    except Exception as e:
    ...
    console.print(f"2. Test the server: [cyan]mcp-agent serve test --script {output}[/cyan]")
    console.print(f"3. Run the server: [cyan]mcp-agent serve --script {output}[/cyan]")
    console.print(f"4. Or serve via HTTP: [cyan]mcp-agent serve --script {output} --transport http --port 8000[/cyan]")