...
SERVER_RECIPES = {
    ...
    "azure": {
        ...
        "category": "cloud",
        "env_required": ["AZURE_SUBSCRIPTION_ID", "AZURE_CLIENT_ID", "AZURE_CLIENT_SECRET"],
    },
...
@app.command("list")
def list_servers(
    available: bool = typer.Option(False, "--available", "-a", help="Show only available servers"),
...
def list_recipes(
    category: Optional[str] = typer.Option(None, "--category", "-c", help="Filter by category"),
    show_env: bool = typer.Option(False, "--show-env", help="Show required environment variables"),
) -> None:
...
@app.command("add")
def add(
    kind: str = typer.Argument(..., help="http|sse|stdio|npx|uvx|recipe|auto"),
    ...
    write: bool = typer.Option(True, "--write/--no-write", help="Persist to config file"),
    force: bool = typer.Option(False, "--force", "-f", help="Overwrite existing server"),
) -> None:
    ...
    if kind == "recipe":
        ...
        if recipe.get("env_required"):
            ...
            if missing:
                console.print(f"[yellow]Warning: Required environment variables not set:[/yellow]")
                for var in missing:
                    console.print(f"  • {var}")
                        ...
                        if tools and tools.tools:
                            ...
                            if verbose:
                                ...
                                if len(tools.tools) > 5:
                                    console.print(f"  [dim]... and {len(tools.tools) - 5} more[/dim]")
                            ...
                            if resources and resources.resources:
                                ...
                                if verbose:
                                    ...
                                    if len(resources.resources) > 5:
                                        console.print(f"  [dim]... and {len(resources.resources) - 5} more[/dim]")
                        except:
...
def import_claude(
    show_only: bool = typer.Option(False, "--show-only", help="Show servers without importing"),
) -> None:
    ...
    if platform.system() == "Darwin":  # macOS
        config_paths = [
            _Path.home() / "Library/Application Support/Claude/claude_desktop_config.json",
        ]
...
def discover_servers() -> None:
    ...
    if platform.system() == "Darwin":
        claude_path = _Path.home() / "Library/Application Support/Claude/claude_desktop_config.json"
