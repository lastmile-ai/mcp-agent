...
class ParallelResultsDisplay:
    ...
    def show_results(self, results: List[tuple[str, str]]) -> None:
        ...
        for i, (model_name, output) in enumerate(results):
            ...
            self.console.print(f"[green]▎[/green] [bold green]{model_name}[/bold green]")
            self.console.print()
    ...
    def show_summary(self, summary: Dict[str, Any]) -> None:
        ...
        if "models" in summary:
            ...
            table.add_row(
                ...
                f"${summary.get('cumulative_cost', 0):.4f}" if "cumulative_cost" in summary else "-"
            )
...
def format_tool_list(tools: List[Any], server_name: Optional[str] = None) -> None:
    ...
    table = Table(title=f"Tools{f' from {server_name}' if server_name else ''}", show_header=True)
...
def format_resource_list(resources: List[Any], server_name: Optional[str] = None) -> None:
    """Format and display a list of resources."""
    if not resources:
        console.print("[yellow]No resources found[/yellow]")
        return
    
    table = Table(title=f"Resources{f' from {server_name}' if server_name else ''}", show_header=True)
