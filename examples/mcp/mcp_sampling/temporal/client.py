...
async def main():
    ...
    app = MCPApp(name="workflow_mcp_client", human_input_callback=console_input_callback)
    async with app.run() as client_app:
        ...
        context.server_registry.registry["demo_server"] = MCPServerSettings(
            name="demo_server",
        ...
        # Connect to the workflow server
        async with gen_client("demo_server", context.server_registry, context=context) as server:
            # Call the BasicAgentWorkflow
            logger.info(f"{type(server)}")
            run_result = await server.call_tool(
                "workflows-HaikuWorkflow-run",
                arguments={
                    "run_parameters": {
            ...
            while True:
                get_status_result = await server.call_tool(
                    "workflows-HaikuWorkflow-get_status",
