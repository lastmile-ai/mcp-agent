...
async def example_usage():
    async with app.run() as agent_app:
        ...
        async with agent:
            ...
            haiku = await llm.generate_str("Write me a haiku about DNS")
            logger.info(f"Generated haiku: {haiku}")

            definition = await llm.generate_str("What does the acronym DNS stand for?")
            logger.info(f"Generated joke: {definition}")
